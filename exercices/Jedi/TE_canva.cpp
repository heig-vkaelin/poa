...
public:
	MainStation(string name, string description)
...
public:
	Line(int number, initializer_list<const Station*> stations)
		: _number(number), _size(stations.size()),
		  _stations(new const Station* [stations.size()]) {
		size_t i = 0;
		for (auto s: stations) {
			_stations[i] = s;
			++i;
...
class Jedi {
public:
	explicit Jedi(string name, const shared_ptr<Jedi>& master = nullptr)
		: _name(move(name)), _master(master) {}
	...
	string toString() const {
		shared_ptr<Jedi> master = _master.lock();
		return _name + ", master: " + (master ? master->_name : "none");
	}
